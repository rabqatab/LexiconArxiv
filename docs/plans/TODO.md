...
- [x] **Corpus gap dashboard** (2026-07-18, commit pending) — `storage.get_corpus_gaps()` + `/api/corpus-gaps` endpoint + a Data Health Monitor section. Shows the top-100 most-cited **enriched** stubs (most-cited papers NOT in corpus — e.g. "Bleu", "Classifier-Free Diffusion Guidance", "Judging LLM-as-a-Judge") linked to DOI/arXiv/OpenAlex, plus a most-referenced-uncollected-venue tally. Server-side `order_by` on the `cited_by_count_internal` index (317 ms, no full scroll); cached 5 min. "Stubs that could be promoted" is covered by the reconcile-stubs feature above rather than a separate live count (corpus-wide promotable scan is expensive).
