...
- [ ] **[Wave 4c Phase 4] Chronological labeling of the shrunk keep-set backlog** — after the demotion the unlabeled non-stub backlog is ~1 M (was ~3.5 M). Label newest→oldest with vLLM (2025-26 smoke → 2020-24 → 2015-19 → 2010-14 → pre-2010). Then the Ollama-partial re-label pass (Issue B below). See plan §Phase 4.
- [ ] **Corpus quality audit — cross-provenance review** (discovered 2026-07-06; partly addressed by Wave 4c 2026-07-09) — Two intertwined issues found during catchup labeling. **Wave 4c update:** the non-CS slice of Issue A is resolved — most non-article junk (books/peer-reviews/editorials) was cross-domain P2/P3 injection and is now demoted to stubs (out of search). What REMAINS: (a) a `type` filter for non-article works *inside* the CS keep-set, and (b) Issue B (Ollama-partial re-label), now scoped to the ~1 M keep-set only.
