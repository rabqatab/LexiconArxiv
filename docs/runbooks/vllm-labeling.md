...
| `ResponseHandlingException: timed out` on **one** `set_payload` inside `batch_update_abstract_structure`, killing the whole 500-paper batch after vLLM had already labeled it | The client-side write loop had no retry — one transient Qdrant timeout under bulk contention lost the entire batch's work (2026-07-04 job b2ab: 573 s, 0 papers saved) | Added `_retry_qdrant_call` with exponential backoff (1→2→4→8→16 s capped at 30 s) around every `set_payload`; commit `c61a652`. Permanent 4xx (validation) still propagates immediately. |
| Multi-vector search 60 s+ timeout while labeling job runs (`"fill query context" timed out after 59.997s`) | Qdrant's default `max_indexing_threads=0` and `max_optimization_threads=null` are both unlimited — background HNSW indexing + segment-merge starves search CPU | Cap both at 2 via the runtime PATCH in [`qdrant-tuning.md`](qdrant-tuning.md). Verified 60 s+ → 1.5-2.3 s. New collections inherit the caps via `ensure_collection_with_vectors` (commit `9636d66`). |
| Two bulk write clients running concurrently (labeling + keywords, e.g.) both crash on Qdrant timeouts | Qdrant cannot handle two heavy write clients + external queries simultaneously — 2026-07-04 proved this at scale | Never parallelize bulk write phases. Chain with sparkq `--after <job-id>`. See [`post-bootstrap-catchup.md`](post-bootstrap-catchup.md) §Bulk write concurrency. |
