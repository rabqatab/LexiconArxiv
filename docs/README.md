...
| [Qdrant Tuning for Mixed Load](./runbooks/qdrant-tuning.md) | Two independent fixes for Qdrant behaviour at corpus scale: (1) CPU tuning (cap `max_indexing_threads`/`max_optimization_threads` so background work stops starving user queries); (2) **payload indices** (added 2026-07-06 — seven new indices that killed the incremental-cycle scroll timeouts on 6.2 M-point filters, plus the diagnostic runbook to spot the same class of bug in future). |
...
|---------|------|---------|
| 0.13.3 | Jul 2026 | Labeling gap + vLLM migration (Path B) + pipeline audit + Qdrant tuning: bulk-vs-incremental systematic audit revealed 7 hidden gaps (labeling/keywords/refs/embed/similarity/graph/topics); post-bootstrap catchup runbook (now serial-execution — 2026-07-04 runtime discovery); Ollama chat retired from pipelines (vLLM primary via `ibm-granite/granite-4.1-8b`), Ollama continues for embedding + search-time HyDE (vector-space integrity); vLLM labeling backend + CLI flag + sparkq launcher + ops runbook; 60-paper quality gate (0.834 Jaccard accepted after multi-label review); throughput gate PASSED at 35.6K/hr @ -p 128 on GB10; `abstract_labeling_gap` DQ warn-check; `embed-papers --priority-tier` for two-phase drain; drain strategy runbook + benchmark script; `_retry_qdrant_call` helper (transient-timeout backoff on `set_payload`); Qdrant `max_indexing/optimization_threads` capped at 2 for search-during-catchup survival (60s→~2s); `QdrantStorage` timeout raised 60s→300s; `[gpu]` extra; docs sweep across abstract_labeling/keyword_extraction/crawling; code overhaul plan queued |
