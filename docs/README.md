...
| [Qdrant Tuning for Mixed Load](./runbooks/qdrant-tuning.md) | When bulk writes are killing search latency: cap `max_indexing_threads`/`max_optimization_threads` so background work stops starving user queries. Verified 60 s+ → ~1.5-2.3 s. |
