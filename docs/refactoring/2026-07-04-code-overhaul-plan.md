...
| **1e-bis. Shared `_retry.py` — consolidation DONE 2026-07-08, coverage extension REMAINING** (commit `c708ae4`, ponytail wave) | `src/core/storage/_retry.py`, `reader.py` | Consolidation shipped: one canonical `retry_qdrant()`; writer/reader/resolver/statistics import it, duplicate helpers deleted, reader→writer directional leak gone. **Still open:** reader.py has 19 raw scroll/count/retrieve calls of which only 9 are wrapped — the remaining 10 bare calls (plus writer bulk writes from 1e proper) are still latent single-timeout fatals. labeling ollama/vllm inline loops intentionally NOT unified (async + domain-specific no-retry branches). |
...
| **1e-quinquies. ~~`--recent-days` on resolve-refs (Step 7)~~ DONE 2026-07-07** (commit `e0cde60`) | `src/core/resolution/resolver.py`, `src/cli/commands/resolution.py` | Shipped in-incident: `fetched_since` threads through all 3 resolver sub-steps + `run_full_pipeline`; CLI gained `--recent-days`; script Step 7 passes it. Step 7 wall time dropped from 7+ h (full 2.8 M sweep) to minutes. |
| **1e-sexies. ~~`graph_indexed` index + Step 9 index-only filter~~ DONE 2026-07-07/08** (commits `2a2d37e`, `293f13b`) | `src/core/storage/statistics.py` | Shipped in-incident: bool index on `graph_indexed` (1.81 M points) + `build_cited_by_incremental` filter rewritten to indexed fields only (`must_not graph_indexed` + `must_not is_stub`) with a client-side `resolved_references` check. 60 s timeout → 0.04 s/page. Pattern to reuse in 1e-ter's lint: an index-only filter with a cheap client-side residual check beats an unindexed `must` clause every time. |
...
| **4b. ~~Drop `--llm` / `--judge` from `extract-keywords`~~ DONE 2026-07-08** (commit `e865217`, ponytail wave) | `src/cli/commands/keywords.py` | Whole keyword-LLM path deleted (ollama/llm_base/judge modules, ABCs, async CLI loop, 636-line test module) after the clean `6283` cycle proved nothing passes the flags. |
| **4c. ~~Dep prune (ponytail audit items #10-13)~~ DONE 2026-07-08** (commits `af7eb07`, `aa11faf`) | `pyproject.toml`, `uv.lock` | `feedparser`, `python-dateutil`, `auto-mix-prep`, `cachetools` dropped; `src/collectors/` deleted first as required. TTLCache → 10-line dict in on_demand.py. |
