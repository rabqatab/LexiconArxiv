...
| **1e-bis. ~~Shared `_retry.py` + coverage across all bulk READ paths~~ DONE 2026-07-08** (commit `c708ae4`, ponytail wave) | `src/core/storage/_retry.py` | Shipped: one canonical `retry_qdrant()` in `src/core/storage/_retry.py`; writer/reader/resolver/statistics all import it (the reader→writer directional leak is gone, and resolver's + statistics' duplicate helpers are deleted). Every reader scroll/count that had per-call retry keeps it via the shared helper. labeling ollama/vllm inline loops intentionally NOT unified (async + domain-specific no-retry branches). |
