...
#   1.2 ACL         - NLP venues
#   1.3 DBLP        - IR/Legal/ACM venues
#   1.4 OpenReview  - ML conferences (ICLR, NeurIPS, ICML)
#   1.5 AAAI        - AAAI proceedings
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "  --skip-dblp          Skip DBLP collection (includes ACM venues)"
...
if [ "$SKIP_OPENALEX" = false ]; then
    ...
    echo "--- [1.1/5] OpenAlex (SKIPPED) ---"
    echo ""
fi

# Step 1.2: ACL Anthology
if [ "$SKIP_ACL" = false ]; then
    echo "--- [1.2/5] ACL Anthology ---"
    ...
    echo "--- [1.2/5] ACL Anthology (SKIPPED) ---"
    echo ""
fi

# Step 1.3: DBLP (includes ACM venues)
if [ "$SKIP_DBLP" = false ]; then
    echo "--- [1.3/5] DBLP (includes ACM venues) ---"
    "$SCRIPT_DIR/collect_dblp.sh" --since-year "$SINCE_YEAR"
    echo ""
else
    echo "--- [1.3/5] DBLP (SKIPPED) ---"
    echo ""
fi

# Step 1.4: OpenReview
if [ "$SKIP_OPENREVIEW" = false ]; then
    echo "--- [1.4/5] OpenReview ---"
    "$SCRIPT_DIR/collect_openreview.sh" --since-year "$SINCE_YEAR"
    echo ""
else
    echo "--- [1.4/5] OpenReview (SKIPPED) ---"
    echo ""
fi
